...
`direct_chain_rect_e8_n119_certificate.py` through the shared
`root_identity_check()`.

For the eighth-power lower bound, Cauchy's inequality over the `120`
positive roots gives
...
2(1-cos z) <= z^2 - z^4/12 + z^6/360,
2 cos z >= 2 - z^2 + z^4/12 - z^6/360 + z^8/30000,
sin(z/2)/(z/2) >= exp(-4z^2/81).
```

Proof.  Put `w=z^2`.  Since `w <= 16 < 56`, the Taylor tails used below
are alternating with decreasing terms from degree `8` onward.
...
The upper bound follows by stopping the same alternating Taylor series after
the positive degree-six term:

```text
2(1-cos z) <= z^2 - z^4/12 + z^6/360.
```

For the cosine lower bound, the alternating expansion gives
...
Let `U_119` be the union of `1/10 x 1/10` cells

```text
R(s0,t0) = [s0,s0+1/10] x [t0,t0+1/10],
s0 in {45, 451/10, ..., 575/10},
t0 in {30, 301/10, ..., 419/10},
```

retained by the positive rational gap in Lemma 5.  The exact enumeration has
`15000` cells.  Each cell lies inside the root cap `|alpha.v| <= 4`
guaranteed by

```text
s,t <= 4^2 * 30 * 119 / (4*248) = 1785/31.
```

Proof.  This is the exact enumeration performed by
`direct_chain_rect_e8_n119_certificate.py`.

## Lemma 5 (local character lower bounds on the union)

For a retained cell `R=[s0,s1] x [t0,t1]` in `U_119`,

```text
chi(y) - chi(x) >= (496/119) g_R,
g_R =
  s0 - t1
  - (248/(6*50*119)) s1^2
  + (248/(6*50*119)) t0^2
  + (1/475)(496^2/1800) s0^3/119^2
  - (1/360)(496^2/1800) t1^3/119^2,
```
...
h_R =
  1 - (s1+t1)/119
  + (1/12)(496/50)(s0^2+t0^2)/119^2
  - (1/360)(496^2/1800)(s1^3+t1^3)/119^3
  + (1/30000)(496^3/300000)(s0^4+t0^4)/119^4.
```

Moreover every retained cell has `g_R > 0`, `h_R > 0`, and
`496^2 h_R^2 - 4 > 0`.

Proof.  Lemmas 1 and 2 give the displayed formulas.  The gap estimate uses
the lower bound for `2(1-cos z)` on the `s` side and the upper bound for
`2(1-cos z)` on the `t` side.  The endpoint choices use lower endpoints in
positive terms and upper endpoints in negative terms.  The exact rational
checker verifies the positivity comparisons over all retained cells.
...
Sine_R = ExpLower((4*248/119)/(81/4) * (s1+t1)),
Neg(119) = 520 * 16^119.
```
...
```text
R119 =
  sum_R A0_lower * 496^119 * 119^(-250)
        * F_R * h_R^119
        * (496^2 h_R^2 - 4)
        * Sine_R
  / Neg(119)
...
Proof.  The checker
`character_ring_iter/direct_chain_rect_e8_n119_certificate.py` performs
these comparisons as exact integer comparisons.  Its current output is

```text
group=E8 n=119 delta=4 e_upper=1457/536
grid_step=1/10 s_start=45 s_end=60 t_start=30 t_end=42
retained_cells=15000 cap=1785/31
log10_union_fraction_lower=-38.16
gap119_min=5527391/5935125
h119_min=7933413921950307734/19828183106689453125 direct_factor119_min=15482442285942823309097622147860483344349596/393156845312405012847483158111572265625
sine_exponent_max=394816/9639
...
positive_gaps: OK
positive_h119: OK
positive_direct119: OK
positive_radial_exp_lower: OK
positive_sine_exp_lower: OK
ratio119_gt_1: OK
```

## Theorem 8 (DCT-RectTrig(E8, n=119))

The rectangular-union positive contribution to `D_E8(119)` dominates the
negative-region bound `520*16^119`.  Hence `D_E8(119) >= 0`.

Proof.  Lemmas 4-6 give the positive rectangular-union lower bound at
`n=119`.  Lemma 7 checks that this lower bound is larger than the
negative-region bound.

## Corollary 9 (conditional E8 odd tail from n=119)

Assume `E8-FiniteBridge(119)`: `Q_3^{E8}(n) >= 0` for every odd
`67 <= n <= 119`.  Then `Q_3^{E8}(n) >= 0` for every odd `n >= 67`.

Proof.  Theorem 8 gives `Q_3^{E8}(121) >= 4 Q_3^{E8}(119)`.  Theorem 8 of
