...
g++ -O3 -DNDEBUG -std=c++20 -fopenmp -Wall -Wextra -Wpedantic \
    ...
    -lgmpxx -lgmp -o "$output_dir/reverse_pieri_checker"

printf 'FULL_Q3_BCD_INDEPENDENT stage=reverse-pieri-complement event=start utc=%s threads=%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$threads" \
    | tee -a "$output_dir/driver.log"
OMP_NUM_THREADS="$threads" stdbuf -oL -eL \
    "$output_dir/reverse_pieri_checker" --progress \
    | tee "$output_dir/reverse_pieri_checker.log"
printf 'FULL_Q3_BCD_INDEPENDENT stage=reverse-pieri-complement event=pass utc=%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$output_dir/driver.log"
...
sha256sum character_ring_iter/verify_full_q3_bcd_modular_moment_checker.cpp \
    ...
    "$output_dir/reverse_pieri_checker.log" \
