...
by `direct_chain_rect_e8_n127_certificate.py`.
...
```text
2(1-cos z) >= z^2 - z^4/12 + z^6/450,
2 cos z >= 2 - z^2 + z^4/12 - z^6/360,
sin(z/2)/(z/2) >= exp(-z^2/21).
```

Proof.  Put `w=z^2`.  Since `w <= 100/9 < 90`, the Taylor tail for
...
```text
1/1800 - w/20160 + w^2/1814400 - w^3/239500800 >= 0
```

on `0 <= w <= 100/9`.  Its derivative is

```text
-1/20160 + w/907200 - w^2/79833600.
```

This derivative is increasing on the interval and at `w=100/9` equals
`-6289/161663040`, so the cubic is decreasing.  Its endpoint value is
`364057/5456127600 > 0`.
...
shows that `F'(x)/x` is decreasing on `0 < x <= 5/3`, because
`5/3 < pi`.  Thus `F` has no interior minimum except at an endpoint.  At
`0`, `F=0`.  At `5/3`, using `333/106 < pi < 22/7 < 10/3`, write
`5/3 = pi/2 + y` with `0 <= y < 61/636`; then

```text
sin(5/3)/(5/3) = cos y /(5/3)
  > (3/5)(1 - (61/636)^2/2)
  = 805271/1348320.
```

For `A=100/189`, the exact rational comparison

```text
(805271/1348320)(1 + A + A^2/2 + A^3/6) - 1
 = 316294764757/27308613054240 > 0
```

and `exp(A) >= 1 + A + A^2/2 + A^3/6` give
`sin(5/3)/(5/3) > exp(-100/189)`.  This is `F(5/3)>0`.
...
Let `U_127` be the union of `1/40 x 1/40` cells

```text
R(s0,t0) = [s0,s0+1/40] x [t0,t0+1/40],
s0 in {40, 1601/40, ...},
t0 in {29, 1161/40, ..., 1279/40},
```

retained by the positive rational gap in Lemma 5.  The exact enumeration has
`10528` cells.  Each cell lies inside the root cap `|alpha.v| <= 10/3`
guaranteed by

```text
s,t <= (10/3)^2 * 30 * 127 / (4*248) = 15875/372.
```

Proof.  This is the exact enumeration performed by
`direct_chain_rect_e8_n127_certificate.py`.

## Lemma 5 (local character lower bounds on the union)

For a retained cell `R=[s0,s1] x [t0,t1]` in `U_127`,

```text
chi(y) - chi(x) >= (496/127) g_R,
g_R =
  s0 - t1
  - (248/(6*50*127)) s1^2
  + (1/450)(496^2/1800) s0^3/127^2,
```
...
h_R =
  1 - (s1+t1)/127
  + (1/12)(496/50)(s0^2+t0^2)/127^2
  - (1/360)(496^2/1800)(s1^3+t1^3)/127^3.
```
...
Sine_R = ExpLower((4*248/(21*127))(s1+t1)),
Neg(127) = 520 * 16^127.
```
...
```text
R127 =
  sum_R A0_lower * 496^127 * 127^(-250)
        * F_R * h_R^127
        * (496^2 h_R^2 - 4)
        * Sine_R
  / Neg(127)
...
Proof.  The checker
`character_ring_iter/direct_chain_rect_e8_n127_certificate.py` performs
...
positive_gaps: OK
positive_h127: OK
positive_direct127: OK
ratio127_gt_1: OK
```

## Theorem 8 (DCT-RectTrig(E8, n=127))

The rectangular-union positive contribution to `D_E8(127)` dominates the
negative-region bound `520*16^127`.  Hence `D_E8(127) >= 0`.

Proof.  Lemmas 4-6 give the positive rectangular-union lower bound at
`n=127`.  Lemma 7 checks that this lower bound is larger than the
negative-region bound.

## Corollary 9 (conditional E8 odd tail from n=127)

Assume `E8-FiniteBridge(127)`: `Q_3^{E8}(n) >= 0` for every odd
`67 <= n <= 127`.  Then `Q_3^{E8}(n) >= 0` for every odd `n >= 67`.

Proof.  Theorem 8 gives `Q_3^{E8}(129) >= 4 Q_3^{E8}(127)`.  Theorem 8 of
