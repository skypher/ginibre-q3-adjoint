...
by `direct_chain_rect_e8_n125_certificate.py`.
...
Proof.  Put `w=z^2`.  Since `w <= 49/4 < 56`, the Taylor tail for
...
on `0 <= w <= 49/4`.  Its derivative is

```text
-1/20160 + w/907200 - w^2/79833600.
```

This derivative is increasing on the interval and at `w=49/4` equals
`-16171/425779200`, so the cubic is decreasing.  Its endpoint value is
`50249/2189721600 > 0`.
...
On `0 < x <= 7/4`, the partial fraction identity

```text
cot x = 1/x - 2x sum_{m>=1} 1/(m^2 pi^2 - x^2)
```

shows that `F'(x)/x` is decreasing.  Hence `F'` changes sign at most once,
so the minimum of `F` on `[0,7/4]` is at an endpoint.  At `0`, `F=0`.  At
`7/4`, using `333/106 < pi < 22/7 < 7/2`, write
`7/4 = pi/2 + y` with `0 <= y < 19/106`; then

```text
sin(7/4)/(7/4) = cos y /(7/4)
  > (4/7)(1 - (19/106)^2/2)
  = 22111/39326.
```

For `A=7/12`, the exact rational comparison

```text
(22111/39326)(1 + A + A^2/2 + A^3/6) - 1
 = 1830085/407731968 > 0
```

and `exp(A) >= 1 + A + A^2/2 + A^3/6` give
`sin(7/4)/(7/4) > exp(-7/12)`.  This is `F(7/4)>0`.
...
Let `U_125` be the union of `1/40 x 1/40` cells

```text
R(s0,t0) = [s0,s0+1/40] x [t0,t0+1/40],
s0 in {44, 1761/40, ...},
t0 in {31, 1241/40, ..., 1339/40},
```

retained by the positive rational gap in Lemma 5.  The exact enumeration has
`8497` cells.  Each cell lies inside the root cap `|alpha.v| <= 7/2`
guaranteed by

```text
s,t <= (7/2)^2 * 30 * 125 / (4*248) = 91875/1984.
```

Proof.  This is the exact enumeration performed by
`direct_chain_rect_e8_n125_certificate.py`.

## Lemma 5 (local character lower bounds on the union)

For a retained cell `R=[s0,s1] x [t0,t1]` in `U_125`,

```text
chi(y) - chi(x) >= (496/125) g_R,
g_R =
  s0 - t1
  - (248/(6*50*125)) s1^2
  + (1/450)(496^2/1800) s0^3/125^2,
```
...
h_R =
  1 - (s1+t1)/125
  + (1/12)(496/50)(s0^2+t0^2)/125^2
  - (1/360)(496^2/1800)(s1^3+t1^3)/125^3.
```
...
Sine_R = ExpLower((4*248/(21*125))(s1+t1)),
Neg(125) = 520 * 16^125.
```
...
```text
R125 =
  sum_R A0_lower * 496^125 * 125^(-250)
        * F_R * h_R^125
        * (496^2 h_R^2 - 4)
        * Sine_R
  / Neg(125)
...
Proof.  The checker
`character_ring_iter/direct_chain_rect_e8_n125_certificate.py` performs
...
positive_gaps: OK
positive_h125: OK
positive_direct125: OK
ratio125_gt_1: OK
```

## Theorem 8 (DCT-RectTrig(E8, n=125))

The rectangular-union positive contribution to `D_E8(125)` dominates the
negative-region bound `520*16^125`.  Hence `D_E8(125) >= 0`.

Proof.  Lemmas 4-6 give the positive rectangular-union lower bound at
`n=125`.  Lemma 7 checks that this lower bound is larger than the
negative-region bound.

## Corollary 9 (conditional E8 odd tail from n=125)

Assume `E8-FiniteBridge(125)`: `Q_3^{E8}(n) >= 0` for every odd
`67 <= n <= 125`.  Then `Q_3^{E8}(n) >= 0` for every odd `n >= 67`.

Proof.  Theorem 8 gives `Q_3^{E8}(127) >= 4 Q_3^{E8}(125)`.  Theorem 8 of
