...
def main() -> int:
    ...
    require(
        "Restoring the fixed $+1$ eigenvalue" in text,
        "the type-B deterministic/random trace cancellation is absent",
    )
    require(
        "\\frac{\\mathfrak a_{d+1}(v)}{\\mathfrak a_d(v)}" in text
        and "\\frac{\\mathfrak b_{d+1}(v)}{\\mathfrak b_d(v)}" in text,
        "the Fredholm order-monotonicity bridge is absent",
    )
    require(
        "semiring cone generated by the single adjoint character" in normalized_text,
        "the corrected adjoint-generated scope statement is absent",
    )
