#include <algorithm>
#include <cstdint>
#include <cstdlib>
#include <iostream>
#include <stdexcept>
#include <vector>

#include <boost/multiprecision/cpp_int.hpp>

namespace {

using Integer = boost::multiprecision::cpp_int;
using Vector = std::vector<Integer>;

Integer binomial_integer(int top, int bottom) {
    if (bottom < 0 || top < bottom) {
        return 0;
    }
    Integer result = 1;
    for (int index = 1; index <= bottom; ++index) {
        result *= top - bottom + index;
        result /= index;
    }
    return result;
}

Integer ordinary_multiplicity(int power, int q, int y) {
    if (y < 0 || y > power * q) {
        return 0;
    }
    Integer result = 0;
    for (int image = 0; image <= power; ++image) {
        const Integer term =
            binomial_integer(power, image)
            * binomial_integer(
                (power - 2 * image) * q - y
                    + power - 2 - image,
                power - 2
            );
        result += image % 2 == 0 ? term : -term;
    }
    return result;
}

Integer folded_multiplicity(
    int power,
    int K,
    int Q,
    int y
) {
    const int period = 2 * K + 2;
    Integer result = 0;
    for (int image = 0; image <= power; ++image) {
        const int direct = image * period + y;
        const int reflected = (image + 1) * period - 1 - y;
        result += ordinary_multiplicity(power, Q, direct);
        result -= ordinary_multiplicity(power, Q, reflected);
        if (
            direct > power * Q
            && reflected > power * Q
        ) {
            break;
        }
    }
    return result;
}

bool adjacent(int K, int Q, int source, int target) {
    return std::abs(source - Q) <= target
        && target <= std::min(
            source + Q,
            2 * K - source - Q
        );
}

Vector multiply(int K, int Q, const Vector& state) {
    Vector next(static_cast<std::size_t>(K + 1));
    for (int source = 0; source <= K; ++source) {
        const Integer& coefficient =
            state[static_cast<std::size_t>(source)];
        if (coefficient == 0) {
            continue;
        }
        for (int target = 0; target <= K; ++target) {
            if (adjacent(K, Q, source, target)) {
                next[static_cast<std::size_t>(target)] += coefficient;
            }
        }
    }
    return next;
}

Integer group_value(
    const std::vector<Vector>& powers,
    int coordinate,
    int target
) {
    const auto f = [&](int power) -> const Integer& {
        return powers[static_cast<std::size_t>(power)][0];
    };
    const auto u = [&](int power) -> const Integer& {
        return powers[static_cast<std::size_t>(power)]
            [static_cast<std::size_t>(coordinate)];
    };
    const auto block = [&](int scalar_power, int walk_power) {
        return
            f(scalar_power) * u(walk_power)
            - f(scalar_power + 1) * u(walk_power - 1);
    };
    if (target == 0) {
        return
            330 * block(4, 8)
            + 462 * block(6, 6)
            + 165 * block(8, 4);
    }
    if (target == 1) {
        return
            385 * block(4, 10)
            + 1254 * block(6, 8)
            + 1122 * block(8, 6);
    }
    return
        2035 * block(6, 10)
        + 4026 * block(8, 8);
}

Vector direct_group_vector(
    int K,
    int Q,
    const std::vector<Vector>& powers,
    int target
) {
    struct WeightedBlock {
        int s;
        long coefficient;
    };
    std::vector<WeightedBlock> blocks;
    int propagation = 0;
    if (target == 0) {
        blocks = {{2, 330}, {3, 462}, {4, 165}};
        propagation = 3;
    } else if (target == 1) {
        blocks = {{2, 385}, {3, 1254}, {4, 1122}};
        propagation = 5;
    } else {
        blocks = {{3, 2035}, {4, 4026}};
        propagation = 7;
    }

    Vector result(static_cast<std::size_t>(K + 1));
    for (const WeightedBlock& block : blocks) {
        const int even = 2 * block.s;
        Vector term(static_cast<std::size_t>(K + 1));
        for (int y = 0; y <= K; ++y) {
            term[static_cast<std::size_t>(y)] =
                powers[static_cast<std::size_t>(even)][0]
                    * powers[
                        static_cast<std::size_t>(9 - even)
                    ][static_cast<std::size_t>(y)]
                - powers[static_cast<std::size_t>(even + 1)][0]
                    * powers[
                        static_cast<std::size_t>(8 - even)
                    ][static_cast<std::size_t>(y)];
        }
        for (int step = 0; step < propagation; ++step) {
            term = multiply(K, Q, term);
        }
        for (int y = 0; y <= K; ++y) {
            result[static_cast<std::size_t>(y)] +=
                block.coefficient
                * term[static_cast<std::size_t>(y)];
        }
    }
    return result;
}

}  // namespace

int main(int argc, char** argv) {
    try {
        if (argc != 2) {
            throw std::runtime_error("usage: MAXIMUM_Q");
        }
        const int maximum_q = std::stoi(argv[1]);
        if (maximum_q < 1) {
            throw std::runtime_error("MAXIMUM_Q must be positive");
        }

        std::uint64_t parameters = 0U;
        std::uint64_t multiplicity_coordinates = 0U;
        std::uint64_t group_coordinates = 0U;
        for (int Q = 1; Q <= maximum_q; ++Q) {
            for (int h = 0; h <= 8 * Q; ++h) {
                const int K = 2 * Q + 1 + h;
                std::vector<Vector> powers(
                    11U,
                    Vector(static_cast<std::size_t>(K + 1))
                );
                powers[0][0] = 1;
                for (int power = 1; power <= 10; ++power) {
                    powers[static_cast<std::size_t>(power)] =
                        multiply(
                            K,
                            Q,
                            powers[
                                static_cast<std::size_t>(power - 1)
                            ]
                        );
                    if (power < 3) {
                        continue;
                    }
                    for (int y = 0; y <= K; ++y) {
                        const Integer expected =
                            folded_multiplicity(power, K, Q, y);
                        const Integer& actual =
                            powers[static_cast<std::size_t>(power)]
                                [static_cast<std::size_t>(y)];
                        if (actual != expected) {
                            std::cerr
                                << "SU2_T4_GROUP_FORMULA_MISMATCH"
                                << " K=" << K
                                << " Q=" << Q
                                << " h=" << h
                                << " power=" << power
                                << " y=" << y
                                << " recurrence=" << actual
                                << " formula=" << expected
                                << '\n';
                            throw std::runtime_error(
                                "finite recurrence disagrees with formula"
                            );
                        }
                        ++multiplicity_coordinates;
                    }
                }
                for (int target = 0; target < 3; ++target) {
                    const Vector direct = direct_group_vector(
                        K,
                        Q,
                        powers,
                        target
                    );
                    for (int y = 0; y <= K; ++y) {
                        const Integer formula =
                            group_value(powers, y, target);
                        const Integer& actual =
                            direct[static_cast<std::size_t>(y)];
                        if (actual != formula) {
                            std::cerr
                                << "SU2_T4_GROUP_VALUE_MISMATCH"
                                << " K=" << K
                                << " Q=" << Q
                                << " h=" << h
                                << " target=" << target
                                << " y=" << y
                                << " direct=" << actual
                                << " formula=" << formula
                                << '\n';
                            throw std::runtime_error(
                                "direct group vector disagrees with formula"
                            );
                        }
                        ++group_coordinates;
                    }
                }
                ++parameters;
            }
        }
        std::cout
            << "SU2_T4_GROUP_FORMULA_AUDIT"
            << " maximum_Q=" << maximum_q
            << " parameters=" << parameters
            << " multiplicity_coordinates="
            << multiplicity_coordinates
            << " group_coordinates=" << group_coordinates
            << " result=PASS_EXACT"
            << '\n';
        return EXIT_SUCCESS;
    } catch (const std::exception& error) {
        std::cerr
            << "SU2_T4_GROUP_FORMULA_AUDIT FAILURE: "
            << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
