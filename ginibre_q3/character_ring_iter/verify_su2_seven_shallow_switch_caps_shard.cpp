...
bool run_cap(
    ...
    const std::filesystem::path log_path =
        log_directory / ("cap_" + std::to_string(cap) + ".log");
    ...
    try {
        if (argc != 5) {
            throw std::runtime_error(
                "usage: TASK_INDEX THREADS LOG_DIRECTORY BEGIN_CAP"
            );
        }
        ...
        if (
            ...
            throw std::runtime_error(
                "invalid task, threads, or begin cap"
            );
        }
        const std::filesystem::path log_directory(argv[3]);
        std::filesystem::create_directories(log_directory);

        const std::size_t count = 128U - begin;
        ...
        for (std::size_t worker = 0U;
            ...
            workers.emplace_back([&]() {
                while (true) {
                    const std::size_t cap = next.fetch_add(1U);
                    if (cap >= 128U) {
                        return;
                    }
                    try {
                        if (!run_cap(task, cap, log_directory)) {
                            ++failed;
                        }
                    } catch (...) {
                        ++failed;
                    ...
                    if (done % 16U == 0U || done == count) {
                        std::cerr
                            << "SU2_SEVEN_SHALLOW_SWITCH_CAP_SHARD"
                            << " task=" << task
                            << " completed=" << done
                            << '/' << count
                            << " failed=" << failed.load()
        ...
        std::cerr
            << "SU2_SEVEN_SHALLOW_SWITCH_CAP_SHARD FAILURE: "
            << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
