...
bool run_task(
    ...
    const std::filesystem::path log_path =
        log_directory
        / ("switch_task_" + std::to_string(index) + ".log");
...
int main(int argc, char** argv) {
    try {
        ...
        indices.reserve(static_cast<std::size_t>(argc - 3));
        for (int argument = 3; argument < argc; ++argument) {
            const std::size_t index =
                parse_size(argv[argument], "task index");
            if (index >= 308U || !seen.insert(index).second) {
                throw std::runtime_error(
        ...
        const std::size_t thread_count =
            std::min(requested_threads, indices.size());
        ...
        for (std::size_t worker = 0U;
            ...
            workers.emplace_back([&]() {
                while (true) {
                    ...
                    std::cerr
                        << "SU2_SEVEN_SHALLOW_SWITCH_TASK_SHARD"
                        << " completed=" << count
                        << '/' << indices.size()
                        << " failed=" << failed.load()
                        << std::endl;
        ...
        std::cerr
            << "SU2_SEVEN_SHALLOW_SWITCH_TASK_SHARD FAILURE: "
            << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
