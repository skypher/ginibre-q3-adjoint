...
constexpr int H = 29;
constexpr int Q_MIN = 15;
constexpr int Q_MAX = 42;
...
std::set<std::string> previous_horizontal_two_strip_shapes(
...
#pragma omp for schedule(dynamic, 64)
    for (std::size_t i = 0; i < items.size(); ++i) {
      ...
      const std::set<std::string> prevs =
          previous_horizontal_two_strip_shapes(shape);
...
int main() {
  ...
  const int b_q_min = env_int("B_Q_MIN", Q_MIN);
  const int b_q_max = env_int("B_Q_MAX", Q_MAX);
  std::vector<Case> b_cases;
  for (int q = Q_MIN; q <= Q_MAX; ++q) {
    if (b_q_min <= q && q <= b_q_max) b_cases.push_back({q, 2 * q + H});
  }

  const std::vector<mpz_class> s = stable_moments(2 * Q_MAX + H);

  std::cout << "BC_TWENTYNINTH_B_FRONTIER_GMP"
            << " h=" << H
            << " q_min=" << Q_MIN
            << " q_max=" << Q_MAX
            << " b_cases=" << b_cases.size()
            << " b_q_min=" << b_q_min
            << " b_q_max=" << b_q_max
