...
#pragma omp parallel for schedule(dynamic, 32)
    for (std::int64_t i = 0; i < static_cast<std::int64_t>(frontier.size()); ++i) {
        ...
        for (Cell cell : cells_of(prefix.part)) {
            ...
            mpz_class value = fixed_top_value(prefix.part, next_fixed, envelope);
            if (value > budget) {
...
int main() {
    constexpr int target_m = 30;
    constexpr int target_n = 2 * target_m - 1;
    constexpr int max_depth = 8;
    constexpr std::size_t expected_over_budget_shapes = 11090;
    constexpr std::array<std::size_t, 7> expected_depth_counts{
        44333, 92312, 60440, 5438, 408, 18, 0};
    const Part expected_best{11, 10, 8, 7, 6, 5, 4, 3, 2, 2, 1};
    const mpz_class expected_max_w("2963381126459933824");
    ...
    std::cout << "B/C fixed-even recursive-envelope m=30 frontier GMP verifier\n"
    ...
    std::size_t m30_over_budget_shapes = 0;
    mpz_class m30_max_w = 0;
    Part m30_best_part;
...
#pragma omp parallel for schedule(dynamic, 128)
        for (std::int64_t i = 0; i < static_cast<std::int64_t>(parts.size()); ++i) {
            ...
            for (Cell e : cells_of(part)) {
                ...
                if (m == target_m && value > budget) {
                    local_frontier.push_back({part, std::vector<Cell>{e}, value});
                }
            }
    ...
    std::cout << "M30_OVER_BUDGET_SHAPES count=" << m30_over_budget_shapes << "\n"
              << "M30_OVER_BUDGET_SHAPES expected_count=" << expected_over_budget_shapes << "\n"
              << "M30_MAX W=" << m30_max_w.get_str()
              << " expected_W=" << expected_max_w.get_str()
              << " bits=" << bit_length(m30_max_w)
              << " shape=" << part_display(m30_best_part) << std::endl;

    std::vector<std::size_t> depth_counts;
    depth_counts.push_back(frontier.size());
    std::cout << "M30_TOP_DEPTH depth=1 count=" << frontier.size()
              << " expected_count=" << expected_depth_counts[0] << std::endl;
    for (int depth = 2; depth <= max_depth && !frontier.empty(); ++depth) {
        frontier = extend_frontier(frontier, budget, envelope);
        depth_counts.push_back(frontier.size());
        std::cout << "M30_TOP_DEPTH depth=" << depth
                  << " count=" << frontier.size()
                  << " expected_count="
    ...
    if (m30_over_budget_shapes != expected_over_budget_shapes) ++failures;
    if (m30_max_w != expected_max_w) ++failures;
    if (m30_best_part != expected_best) ++failures;
    if (m30_max_w <= budget) ++failures;
    ...
    for (std::size_t i = 0; i < frontier.size() && i < display_limit; ++i) {
        ...
        std::cout << "M30_REMAINING_PREFIX shape=" << part_display(row.part)
                  << " fixed=" << fixed_display(row.fixed)
                  << " W_top" << row.fixed.size() << "=" << row.value.get_str()
                  << " exceeds_budget=yes\n";
    }
