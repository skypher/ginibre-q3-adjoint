#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=127.

This checks the arithmetic part of the DCT-RectTrig(E8, n=127) leaf.  The
region is a 1/40 by 1/40 rectangular union inside the wider root angle cap
|alpha.v| <= 10/3.  The local input is the same E8 quartic-sextic package,
with the gap coefficient weakened to 1/450 so that the scalar bound remains
valid on the wider cap.
...
D_E8(127) = Q_3^E8(129) - 4 Q_3^E8(127) with the negative-region bound
520*16^127.
"""
...
def main() -> None:
    ...
    if len(cells) != 10528:
        raise AssertionError(f"expected 10528 retained grid cells, got {len(cells)}")
    ...
    checks = {
        ...
        "cell_count_10528": len(cells) == 10528,
        "all_cells_in_cap": all(s1 < cap_value() and t1 < cap_value() for _, s1, _, t1 in cells),
        "positive_gaps": min(gaps) > 0,
        "positive_h127": min(h_values) > 0,
        "positive_direct127": min(direct_values) > 0,
    ...
    print(f"gap127_min={min(gaps)}")
    print(f"h127_min={min(h_values)} direct_factor127_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio127_lower={log10_fraction(ratio127):.2f}")
    print(f"log10_best_cell_ratio127={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=127 rectangular certificate failed")
