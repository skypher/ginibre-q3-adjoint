...
constexpr int B_ABSORPTION_Q = 48;
constexpr int C_ABSORPTION_Q = 69;
...
mpz_class c_twentyseventh_pause_polynomial(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
...
mpz_class c_twentyseventh_fpf_pause_bound(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
  for (int pauses = H % 2; pauses <= H; pauses += 2) {
...
mpq_class b_twentyseventh_lower_box_rational() {
  mpq_class out(0);
...
int main() {
  ...
  int failures = 0;
  std::vector<Case> all_b_cases;
  for (int q = 15; q < B_ABSORPTION_Q; ++q) {
    all_b_cases.push_back({q, 2 * q + H});
  }
  ...
  std::cout << "BC_TWENTYSEVENTH_FRONTIER_GMP b_cases=" << b_cases.size()
  ...
  const mpq_class lower_box_ratio = b_twentyseventh_lower_box_rational();
...

  {
    ...
    const mpz_class p = c_twentyseventh_pause_polynomial(q);
  ...
  if (run_c) {
    for (int q : c_fpf_q_cases) {
      const int j = 2 * q + H;
      const mpz_class bound = c_twentyseventh_fpf_pause_bound(q);
    ...
    for (int q : c_q_cases) {
      ...
      if (margin < 0) {
        ++failures;
        std::cout << "C_FAIL twentyseventh_pause q=" << q
                  << " j=" << j
                  << " pause_polynomial=" << p
                  << " stable=" << s[j] << "\n";
      }
