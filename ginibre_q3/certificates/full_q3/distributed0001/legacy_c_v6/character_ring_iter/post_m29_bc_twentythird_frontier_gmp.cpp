...
constexpr int B_ABSORPTION_Q = 40;
constexpr int C_ABSORPTION_Q = 59;
...
mpz_class c_twentythird_pause_polynomial(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
...
mpz_class c_twentythird_fpf_pause_bound(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
  for (int pauses = H % 2; pauses <= H; pauses += 2) {
...
mpq_class b_twentythird_lower_box_rational() {
  mpq_class out(0);
...
int main() {
  ...
  const std::vector<int> c_fpf_q_cases = {21, 22, 23, 24, 25, 26, 27, 28};
  const std::vector<int> c_q_cases = {29, 30, 31, 32, 33, 34, 35,
  ...
  std::cout << "BC_TWENTYTHIRD_FRONTIER_GMP b_cases=" << b_cases.size()
  ...
  const mpq_class lower_box_ratio = b_twentythird_lower_box_rational();
...

  {
    ...
    const mpz_class p = c_twentythird_pause_polynomial(q);
  ...
  if (run_c) {
    for (int q : c_fpf_q_cases) {
      const int j = 2 * q + H;
      const mpz_class bound = c_twentythird_fpf_pause_bound(q);
    ...
    for (int q : c_q_cases) {
      ...
      if (margin < 0) {
        ++failures;
        std::cout << "C_FAIL twentythird_pause q=" << q
                  << " j=" << j
                  << " pause_polynomial=" << p
                  << " stable=" << s[j] << "\n";
      }
