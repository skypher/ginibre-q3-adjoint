...
mpz_class c_ninth_pause_polynomial(int q) {
  const int j = 2 * q + 9;
  return mpz_class(1024) * binom(j, 1)
      + mpz_class(4096) * binom(j, 3)
      + mpz_class(16384) * binom(j, 5)
      + mpz_class(65536) * binom(j, 7)
      + mpz_class(262144) * binom(j, 9);
}
...
int main() {
  ...
  const std::vector<int> c_q_cases = {21, 22};
  const std::vector<mpz_class> s = stable_moments(53);

  std::cout << "BC_NINTH_FRONTIER_GMP b_cases=" << b_cases.size()
  ...
  for (int q : c_q_cases) {
    const int j = 2 * q + 9;
    const mpz_class p = c_ninth_pause_polynomial(q);
    const mpz_class bound = pow2(2 * q + 1) * p * s[q + 9];
    const mpz_class margin = s[j] - bound;
    if (margin < 0) {
      ++failures;
      std::cout << "C_FAIL ninth_pause q=" << q
                << " j=" << j
                << " pause_polynomial=" << p
                << " stable=" << s[j] << "\n";
    }
