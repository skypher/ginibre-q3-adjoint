#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=129.

This checks the arithmetic part of the DCT-RectTrig(E8, n=129) leaf.  The
region is a 1/40 by 1/40 rectangular union inside the root angle cap
|alpha.v| <= 19/6, using the same quartic-sextic trigonometric package as
the n=131 bridge certificate.

The new input at n=129 is a sharper rational exponential lower bound.  For
`x = k+r`, `k=floor(x)`, and `0 <= r < 1`, the checker uses

    exp(-x) >= (1457/536)^(-k) * (1 - r + r^2/2 - r^3/6)

for the radial Gaussian and Weyl sine density losses.  The script compares
the resulting rectangular-union lower bound for
D_E8(129) = Q_3^E8(131) - 4 Q_3^E8(129) with the negative-region bound
520*16^129.
"""
...
def main() -> None:
    ...
    if len(cells) != 12719:
        raise AssertionError(f"expected 12719 retained grid cells, got {len(cells)}")
    ...
    checks = {
        ...
        "cell_count_12719": len(cells) == 12719,
        "all_cells_in_cap": all(s1 < cap_value() and t1 < cap_value() for _, s1, _, t1 in cells),
        "positive_gaps": min(gaps) > 0,
        "positive_h129": min(h_values) > 0,
        "positive_direct129": min(direct_values) > 0,
    ...
    print(f"gap129_min={min(gaps)}")
    print(f"h129_min={min(h_values)} direct_factor129_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio129_lower={log10_fraction(ratio129):.2f}")
    print(f"log10_best_cell_ratio129={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=129 rectangular certificate failed")
