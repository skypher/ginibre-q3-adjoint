...
constexpr int B_ABSORPTION_Q = 46;
constexpr int C_ABSORPTION_Q = 66;
...
mpz_class c_twentysixth_pause_polynomial(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
...
mpz_class c_twentysixth_fpf_pause_bound(int q) {
  const int j = 2 * q + H;
  mpz_class out = 0;
  for (int pauses = H % 2; pauses <= H; pauses += 2) {
...
mpq_class b_twentysixth_lower_box_rational() {
  mpq_class out(0);
...
int main() {
  ...
  std::cout << "BC_TWENTYSIXTH_FRONTIER_GMP b_cases=" << b_cases.size()
  ...
  const mpq_class lower_box_ratio = b_twentysixth_lower_box_rational();
...

  {
    ...
    const mpz_class p = c_twentysixth_pause_polynomial(q);
  ...
  if (run_c) {
    for (int q : c_fpf_q_cases) {
      const int j = 2 * q + H;
      const mpz_class bound = c_twentysixth_fpf_pause_bound(q);
    ...
    for (int q : c_q_cases) {
      ...
      if (margin < 0) {
        ++failures;
        std::cout << "C_FAIL twentysixth_pause q=" << q
                  << " j=" << j
                  << " pause_polynomial=" << p
                  << " stable=" << s[j] << "\n";
      }
