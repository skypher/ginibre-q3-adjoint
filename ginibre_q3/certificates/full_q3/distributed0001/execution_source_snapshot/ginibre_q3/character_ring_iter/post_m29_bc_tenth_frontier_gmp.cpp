...
mpz_class c_tenth_pause_polynomial(int q) {
  const int j = 2 * q + 10;
  mpz_class out = 0;
  for (int pauses = 0; pauses <= 10; pauses += 2) {
    out += pow2(10 + pauses) * binom(j, pauses);
  }
  return out;
}
...
int main() {
  ...
  const std::vector<mpz_class> s = stable_moments(60);

  std::cout << "BC_TENTH_FRONTIER_GMP b_cases=" << b_cases.size()
  ...
  for (int q : c_q_cases) {
    const int j = 2 * q + 10;
    const mpz_class p = c_tenth_pause_polynomial(q);
    const mpz_class bound = pow2(2 * q + 1) * p * s[q + 10];
    const mpz_class margin = s[j] - bound;
    if (margin < 0) {
      ++failures;
      std::cout << "C_FAIL tenth_pause q=" << q
                << " j=" << j
                << " pause_polynomial=" << p
                << " stable=" << s[j] << "\n";
    }
