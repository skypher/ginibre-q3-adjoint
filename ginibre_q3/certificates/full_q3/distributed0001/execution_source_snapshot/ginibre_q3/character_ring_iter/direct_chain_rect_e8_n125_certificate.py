#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=125.

This checks the arithmetic part of the DCT-RectTrig(E8, n=125) leaf.  The
region is a 1/40 by 1/40 rectangular union inside the root angle cap
|alpha.v| <= 7/2.  The local input is the same E8 quartic-sextic package,
with the gap coefficient 1/450.
...
D_E8(125) = Q_3^E8(127) - 4 Q_3^E8(125) with the negative-region bound
520*16^125.
"""
...
def main() -> None:
    ...
    if len(cells) != 8497:
        raise AssertionError(f"expected 8497 retained grid cells, got {len(cells)}")
    ...
    print(f"gap125_min={min(gaps)}")
    print(f"h125_min={min(h_values)} direct_factor125_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio125_lower={log10_fraction(ratio125):.2f}")
    print(f"log10_best_cell_ratio125={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=125 rectangular certificate failed")
