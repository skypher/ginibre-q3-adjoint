#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=119.

This checks the arithmetic part of the DCT-RectTrig(E8, n=119) leaf.  The
region is a 1/10 by 1/10 rectangular union inside the root angle cap
|alpha.v| <= 4.  Compared with the n=121 certificate, the gap lower bound
also uses the matching upper estimate

    2(1-cos z) <= z^2 - z^4/12 + z^6/360

on the smaller-radius side of the difference.

The local h input uses the E8 quartic-sextic package and the Cauchy
eighth-moment lower bound

    sum (alpha.u)^8 >= Q(u)^4 / 300000.
...
D_E8(119) = Q_3^E8(121) - 4 Q_3^E8(119) with the negative-region bound
520*16^119.
"""
...
GAP6_LOWER = Fraction(1, 475)
COS4_COEFF = Fraction(1, 12)
COS6_LOSS = Fraction(1, 360)
COS8_COEFF = Fraction(1, 30000)
...
def gap_lower(s0: Fraction, s1: Fraction, t0: Fraction, t1: Fraction) -> Fraction:
    ...
    sixth_scale = Fraction((2 * D) ** 2, SEXTIC_DENOM)
    return (
        s0
        - t1
        - rho * s1 * s1
        + rho * t0 * t0
        + GAP6_LOWER * sixth_scale * s0**3 / (N * N)
        - COS6_LOSS * sixth_scale * t1**3 / (N * N)
    )
...
def local_h_lower(s0: Fraction, s1: Fraction, t0: Fraction, t1: Fraction) -> Fraction:
    ...
    eighth_gain = COS8_COEFF * Fraction((2 * D) ** 3, EIGHTH_LOWER_DENOM)
    return (
        Fraction(1)
        - (s1 + t1) / N
        + quartic_gain * (s0 * s0 + t0 * t0) / (N * N)
        - sixth_loss * (s1**3 + t1**3) / (N**3)
        + eighth_gain * (s0**4 + t0**4) / (N**4)
    )
...
def main() -> None:
    ...
    if len(cells) != 15000:
        raise AssertionError(f"expected 15000 retained grid cells, got {len(cells)}")
    ...
    sine_exponents = [
        Fraction(4 * D, N) * (s1 + t1) / SINE_EXP_DENOM
        for _, s1, _, t1 in cells
    ]
    a0 = a0_lower()
    ratio119 = ratio_lower(cells, a0)
    ...
    checks = {
        ...
        "cell_count_15000": len(cells) == 15000,
        "all_cells_in_cap": all(s1 < cap_value() and t1 < cap_value() for _, s1, _, t1 in cells),
        "positive_gaps": min(gaps) > 0,
        "positive_h119": min(h_values) > 0,
        "positive_direct119": min(direct_values) > 0,
    ...
    print(f"gap119_min={min(gaps)}")
    print(f"h119_min={min(h_values)} direct_factor119_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio119_lower={log10_fraction(ratio119):.2f}")
    print(f"log10_best_cell_ratio119={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=119 rectangular certificate failed")
