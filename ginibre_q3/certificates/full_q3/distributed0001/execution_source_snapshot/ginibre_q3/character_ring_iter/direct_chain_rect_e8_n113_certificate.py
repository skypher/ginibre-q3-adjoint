#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=113.

This checks the arithmetic part of the DCT-RectTrig(E8, n=113) leaf.  The
positive region is a 1/20 by 1/20 rectangular union inside the root angle
cap |alpha.v| <= 4.  It uses the same cubic gap and average-character scalar
polynomials as the n=117 bridge step and the quartic Weyl-sine loss from the
n=115 bridge step.

The negative-region bound is sharpened using the exact E8 adjoint moments
m_0..m_6:

    int (X-Y)^2 (X+Y)^2 ((X+Y)^2+4) dX dY = 176.

Since |X+Y| <= 16, this gives
...
LOW_MOMENTS = (1, 0, 1, 1, 5, 16, 79)
NEGATIVE_MOMENT_CONSTANT = 176
...
def parse_low_moments(path: Path) -> tuple[int, ...]:
    ...
    return tuple(moments[k] for k in range(len(LOW_MOMENTS)))


def low_moment_source_check() -> bool:
    here = Path(__file__).resolve().parent
    log_path = here / "logs" / "e8_70.log"
    oeis_path = here.parent / "references" / "oeis_A179663.txt"
    expected = tuple(LOW_MOMENTS)
    return parse_low_moments(log_path) == expected and parse_low_moments(oeis_path) == expected
...
def moment_integral(power: int) -> int:
    ...
    for k in range(power + 1):
        coeff = comb(power, k)
        total += coeff * moment[k + 2] * moment[power - k]
        total += coeff * moment[k] * moment[power - k + 2]
        total -= 2 * coeff * moment[k + 1] * moment[power - k + 1]
    return total


def low_moment_negative_check() -> bool:
    return (
        moment_integral(0) == 2
        and moment_integral(2) == 8
        and moment_integral(4) == 144
        and moment_integral(4) + 4 * moment_integral(2) == NEGATIVE_MOMENT_CONSTANT
    )
...
def negative_bound() -> int:
    return NEGATIVE_MOMENT_CONSTANT * (2 * C_NEG) ** (N - 2)
...
def main() -> None:
    ...
    if len(cells) != 91581:
        raise AssertionError(f"expected 91581 retained grid cells, got {len(cells)}")
    ...
    ratio113 = sum(positive_terms) / neg
    ...
    checks = {
        **cubic_checks,
        ...
        "low_moment_sources": low_moment_source_check(),
        "low_moment_negative_176": low_moment_negative_check(),
        "cell_count_91581": len(cells) == 91581,
        "all_cells_in_cap": all(s1 < cap_value() and t1 < cap_value() for _, s1, _, t1 in cells),
        "positive_gaps": min(gaps) > 0,
        "positive_h113": min(h_values) > 0,
        "positive_direct113": min(direct_values) > 0,
    ...
    print(f"negative_bound={NEGATIVE_MOMENT_CONSTANT}*16^{N - 2}")
    print(f"log10_union_fraction_lower={log10_fraction(sum(rect_fracs)):.2f}")
    print(f"gap113_min={min(gaps)}")
    print(f"h113_min={min(h_values)} direct_factor113_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio113_lower={log10_fraction(ratio113):.2f}")
    print(f"log10_best_cell_ratio113={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=113 rectangular certificate failed")
