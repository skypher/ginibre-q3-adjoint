...
int main(int argc, char** argv) {
    ...
    if (onset_log.empty()
        ...
        || b_rank_hi < b_rank_lo
        || c_rank_hi < c_rank_lo) {
        std::cerr << "invalid B/C Pieri-compression GMP verifier arguments\n";
        return 2;
    }
    ...
    std::cout << "B/C residual Pieri-compression 2^q-loss arithmetic GMP verifier\n"
              << "VERIFIES exact inequality: 2^(q+1)*binom(j,q)*s_{j-q} <= s_j, q=rank+1\n"
              << "This is the arithmetic implication of a possible 2^q-loss tableaux supplier;\n"
              << "it does not by itself prove the combinatorial compression inequality.\n"
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int row_slot = 0; row_slot < static_cast<int>(rows.size()); ++row_slot) {
        ...
        mpz_ui_pow_ui(pow2_factor.get_mpz_t(), 2UL, static_cast<unsigned long>(q));
        ...
        for (int moment = boundary; moment <= onset + 2; ++moment) {
            const mpz_class bound = pow2_factor * binom * stable[static_cast<std::size_t>(moment - q)];
