#!/usr/bin/env python3
"""Exact-rational certificate for the E8 direct-Chain bridge step at n=121.

This checks the arithmetic part of the DCT-RectTrig(E8, n=121) leaf.  The
region is a 1/30 by 1/30 rectangular union inside the root angle cap
|alpha.v| <= 4.  The local input extends the E8 quartic-sextic package by
the Cauchy eighth-moment lower bound
...
D_E8(121) = Q_3^E8(123) - 4 Q_3^E8(121) with the negative-region bound
520*16^121.
"""
...
def main() -> None:
    ...
    if len(cells) != 74053:
        raise AssertionError(f"expected 74053 retained grid cells, got {len(cells)}")
    ...
    checks = {
        ...
        "cell_count_74053": len(cells) == 74053,
        "all_cells_in_cap": all(s1 < cap_value() and t1 < cap_value() for _, s1, _, t1 in cells),
        "positive_gaps": min(gaps) > 0,
        "positive_h121": min(h_values) > 0,
        "positive_direct121": min(direct_values) > 0,
    ...
    print(f"gap121_min={min(gaps)}")
    print(f"h121_min={min(h_values)} direct_factor121_min={min(direct_values)}")
    print(f"sine_exponent_max={max(sine_exponents)}")
    print(f"log10_ratio121_lower={log10_fraction(ratio121):.2f}")
    print(f"log10_best_cell_ratio121={log10_fraction(best_term):.2f}")
    for name, ok in checks.items():
        print(f"{name}: {'OK' if ok else 'FAIL'}")
    if not all(checks.values()):
        raise AssertionError("E8 n=121 rectangular certificate failed")
