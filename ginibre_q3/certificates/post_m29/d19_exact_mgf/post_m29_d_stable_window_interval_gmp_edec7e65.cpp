...
ExactCorrections read_exact_correction_logs(
    ...
    const std::regex full_pattern(
        R"(^\s*D_(\d+) m_(\d+) \+= O_even (-?\d+) \+ det (-?\d+); Delta_(\d+) = (-?\d+))"
    );
    ...
    for (const std::string& path : paths) {
        ...
        while (std::getline(input, line)) {
            ...
            int index = 0;
            mpz_class value;
            if (std::regex_search(line, match, full_pattern)) {
        ...
        if (!success) {
            std::cerr << path << " does not record __EXIT_STATUS=0\n";
            std::exit(2);
        }
        if (!found_usable_row) {
...
mpz_class ceil_sqrt(const mpz_class& value) {
    if (value < 0) {
        std::cerr << "cannot take a square root of a negative integer\n";
        std::exit(2);
    }
    mpz_class root;
    mpz_sqrt(root.get_mpz_t(), value.get_mpz_t());
    if (root * root != value) ++root;
    return root;
}
...
std::vector<mpz_class> determinant_upper_bounds(
    ...
    for (int index = 0; index <= maximum; ++index) {
        if (index % 2 == 0) {
            ...
            upper[static_cast<std::size_t>(index)] = ceil_sqrt(
...
Interval correction_interval(
    ...
    if (index < rank) {
        ...
        // obeys B_j<=s_j for even j and, by Cauchy--Schwarz,
        // B_j^2<=s_{j-1}s_{j+1} for odd j.
...
constexpr std::array<ExactPrefixRank, 21> D4_24_EXACT_PREFIXES{{
    ...
    {16, 38}, {17, 38}, {18, 36}, {19, 34}, {20, 30}, {21, 30},
    {22, 30}, {23, 30}, {24, 30},
}};
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        std::cout << "D4-D24 exact-prefix/two-sided-suffix GMP interval certificate\n"
                  ...
                  << "j>2rank:[-s_j,U_j]\n"
                  << "upper_bound=U_even:s_j,U_odd:ceil_sqrt(s_(j-1)s_(j+1))\n";
#ifdef _OPENMP
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    std::cout << "D stable-window/two-sided suffix GMP interval audit\n"
              ...
              << "UPPER_BOUND_STATUS: even/Cauchy determinant bound\n"
