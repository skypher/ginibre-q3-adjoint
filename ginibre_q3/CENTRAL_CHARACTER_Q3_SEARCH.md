...
at `F_0(d)`.  Since `d_a=1`, every odd channel contains `a` and every bracket
graph in it has a unique edge incident with `a`.  Removing that edge gives
...
strictly stronger but more symmetric target.  Equivalently, for a section
`s:image(phi)->F_0(d)`, the defect

```text
D_s=(1-s phi)iota_+:P_p->K                          (P22.5h)
```

proves `(UKS)` whenever `A intersect K` is contained in `image(D_s)` and
`dim ker(D_s)>=alpha_p`: the rank of `D_s` pays for the carrier and its
kernel pays for the allocation directions.
...
defect: one perturbed section, together with the pair and four-term
relations, has exact rank 19.  In the notation of `(P22.5e)`, the analyzer
finds
...
`S_(2,1,1,1) U`.  The face-only residual in `(P22.5i)` is a 12-term
...
both the recovered relation span and `(P22.5g)` in every case.  The remaining
uniform task is to prove `(P22.5g)` (or construct one section satisfying the
criterion after `(P22.5h)`) in arbitrary multidegree.  The exact commands,
